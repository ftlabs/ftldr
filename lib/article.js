...
function extractData(uuid) {
  ...
  .then(content => {
    ...
    const paragraphs = content.bodyXML.match(/<p>(.*?)<\/p>/g);
    ...
    if( type ){
      ...
      if (type.prefLabel === 'Opinion') {
        const lastSentence = paragraphs[paragraphs.length-1];
        const penultimateSentence = paragraphs[paragraphs.length-2];
        essencePhrases.push(
          {'type': 'Last sentence', text: extractText(lastSentence)},
          {'type': 'Penultimate sentence', text: extractText(penultimateSentence)}
        );
      }
