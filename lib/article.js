  ...
  .then(content => {
    ...
    if( ! genre ){
      ...
      if (genre.prefLabel === 'News') {
        ...
        const mailtoIndex = allParagraphs.findIndex( p => { return p.match(/mailto[\s\S]*@ft\.com/); });
        const parasBeforeMailto = (mailtoIndex > -1)? allParagraphs.slice(0,mailtoIndex) : AllParagraphs;
